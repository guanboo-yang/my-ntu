...
export const handler: Handler = async (event, _context) => {
	...
	switch (event.httpMethod) {
		...
		case 'POST':
			...
			const res = await fetch(cookieJar, 'https://if163.aca.ntu.edu.tw/eportfolio/student/index.asp')
			const arrayBuffer = await res.arrayBuffer()
			const decoded = decoder.decode(arrayBuffer)
			const $ = cheerio.load(decoded)
			const el = $('body > table:nth-child(2) > tbody > tr > td.content > div > div > div > table > tbody')
			...
			profile.birthday = el.find('tr:nth-child(4) > td:nth-child(2)').text().trim()
			profile.idNumber = el.find('tr:nth-child(4) > td:nth-child(4)').text().trim()
