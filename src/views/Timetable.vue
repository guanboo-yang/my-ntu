<template>
	<v-table fixed-header height="calc(100vh - 112px)">
		...
		<tbody>
			<tr v-for="row in 15" :key="row">
				...
				<template v-for="col in 6" :key="col">
					<template v-for="t in [timetable[row + col * 15 - 16]]">
						<td v-if="t === undefined" />
						<td v-else-if="t" :rowspan="t.time.length" :style="`background-color: ${getCourseColor(t.name)}; ${t.visit && 'opacity: 0.5'}`" @click="showInfo(t)">
							<p>{{ t.name }}</p>
			...
			<tr v-if="noTime">
				<td :colspan="showDays === 5 ? 6 : 7">「{{ noTime?.map(n => n.name).join('、') }}」因上課時間特殊而未能顯示於時間表，請留意上課時間並避免衝堂。</td>
			</tr>
		</tbody>
		<v-dialog v-model="dialog">
			<v-card v-if="course" width="300" style="margin: auto">
				<v-card-item>
					<v-icon :icon="mdiClose" size="small" style="float: right; margin: 4px" @click="dialog = false" />
					<v-card-title style="padding-bottom: 0">{{ course.name }}</v-card-title>
					<p style="font-size: 14px">
						<span>{{ course.location }}</span>
						<span class="dot" v-if="course.professor">{{ course.professor }}</span>
						<span class="dot" v-if="course.credits">{{ course.credits }}學分</span>
						<span class="dot" v-if="course.visit">旁聽</span>
						<!-- <span>{{ [course.location, course.professor].filter(Boolean).join(' ・ ') }}</span> -->
					</p>
					<v-divider style="margin: 8px 0" />
					<div style="margin: -4px 0 8px 0">
						<v-chip-group disabled column variant="tonal">
							<v-chip v-for="link in course.links" :key="link.url" size="small" :style="`color: ${linkToColor(link.name)}`" :href="link.url" target="_blank" rel="noreferrer">
								{{ link.name }}
							</v-chip>
						</v-chip-group>
					</div>
		...
		<v-dialog v-model="choose">
			<v-card width="300" style="margin: auto">
				<v-card-text style="text-align: center">
					...
					<v-select v-model="semester" :items="semestersOptions" variant="outlined" density="comfortable" hide-details />
					<v-switch v-model="showDays" :falseValue="5" :trueValue="6" label="顯示星期六" inset hide-details />
				</v-card-text>
			</v-card>
		</v-dialog>
	</v-table>
...
<script setup lang="ts">
	...
	const timeStamp = ['07:10', '08:10', '09:10', '10:20', '11:20', '12:20', '13:20', '14:20', '15:30', '16:30', '17:30', '18:25', '19:20', '20:15', '21:10']
	...
	const semestersOptions = computed(() =>
		Object.keys(semesters)
			.map(s => ({
				title: `${semesters[s].year}學年 ${semesters[s].semester === 1 ? '上' : '下'}學期`,
				value: s,
			}))
	...
	const noTime = computed(() => {
		const noTime = semesters[semester.value].courses.filter(c => !parseInfo(c.info).length)
